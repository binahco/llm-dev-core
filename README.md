...
cassettes congelados (`test-kit` + `ci-pack`), la web de un consumidor sale de una base
(`web-api-base`) y el archivo real entra normalizado (secciones/tablas con mapeo a
línea, `parser-io`).
...
uv run pytest  # tests (107 a la fecha)
```