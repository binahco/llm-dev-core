...
- **Empaquetado:** una sola dist `llm-dev-core` (D1): `llm_client`, `schema_validate`, `secure_base` y `test_kit` top-level; los consumidores dependen de `llm-dev-core ^0.x`, nunca de módulos sueltos.
...
uv run pytest  # tests (49 a la fecha)
```